...
public class ModelBehaviorStrategy extends AbstractCompletionStrategy implements ICompletionStrategy {

	public ModelBehaviorStrategy(ICompletionContext context) {
		super(context);
	}

	@Override
	public void apply(ICompletionReporter reporter) throws Exception {
		ModelBehaviorContext context = (ModelBehaviorContext) this.getContext();
		...
		for(IScriptFolder folder : context.getSourceModule().getScriptProject().getScriptFolders()){
			if(!folder.getPath().toString().endsWith("/behaviors")) continue;
			for(IModelElement source : folder.getChildren()){
				if(source instanceof SourceModule){
					...
					try{
						...
						if(className.toLowerCase().startsWith(param.toLowerCase()) && className.endsWith("Behavior")){
							reporter.reportKeyword(className.substring(0, className.lastIndexOf("Behavior")), "", this.getReplacementRange(context));
						}
					}catch(Exception e){
					}
				}
