...
public class ControllerComponentsStrategy extends AbstractCompletionStrategy  implements ICompletionStrategy {
	public ControllerComponentsStrategy(ICompletionContext context) {
		super(context);
	}
	
	@Override
	public void apply(ICompletionReporter reporter) throws Exception {
		ControllerComponentsContext context = (ControllerComponentsContext) this.getContext();
		...
		for(IScriptFolder folder : context.getSourceModule().getScriptProject().getScriptFolders()){
			if(!folder.getPath().toString().endsWith("/components")) continue;
			for(IModelElement source : folder.getChildren()){
				if(source instanceof SourceModule){
					...
					try{
						...
						if(className.toLowerCase().startsWith(param.toLowerCase()) && className.endsWith("Component")){
							reporter.reportKeyword(className.substring(0, className.lastIndexOf("Component")), "", this.getReplacementRange(context));
						}
					}catch(Exception e){
					}
				}
